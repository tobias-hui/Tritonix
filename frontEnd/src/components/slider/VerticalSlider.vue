...
function handleClick(swiper: Swiper, e: MouseEvent | TouchEvent | PointerEvent) {
  // e.stopPropagation()
...
<template>
  <div style="padding:0 10px 0;background-color: pink;">
    ...
    <div class="slide-wrapper" ref="wrapperRef" @mousedown="handleMouseDown" @mouseup="handleMouseUp"
      @mousemove="handleMouseMove">

      <div class="slide-list" ref="slideListRef"
        :style="{ transform: `translate3d(0,${slideState.transition.y}px,0)`, transitionDuration: `${slideState.durationTime}s` }">
        <SlideItem v-for="( item, index ) in  dataList " :key="item.id" :slider-index="index" :src="item.url" :position="{
          left: 0,
